...
#function to relativize data
relativize_data<- function(mtt_matrix)
{
  #mean of the control groups
  mean(c(mtt_matrix[,2], mtt_matrix[,11]))
  #relativize abs data
...
#plot the data
barplot(relative_means, main = "viability", xlab="ratio", ylab = "cell viability (%)", names.arg = c(rep("10:1",2), rep("8:1", 2), rep("5:1", 2), rep("2:1", 2), rep("Control", 2)))